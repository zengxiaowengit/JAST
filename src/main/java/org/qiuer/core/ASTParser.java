...
public class ASTParser {
  ...
  private static void getClasses(String packageName, String astPath, final boolean recursive, HashMap<String, Class> classes){
    ...
    for (File file : dirfiles) {
      if (file.isDirectory()) {
        getClasses(packageName + "." + file.getName(), file.getAbsolutePath(), recursive,
                classes);
      } else {
   ...
   * @return
   */
  public Object parse(Map<String, Object> tree){
    if (tree == null)
      return null;
...
//    fields.addAll(Arrays.asList(clazz.getFields()));
      ...
      tree.forEach((key, value)->{
        ...
        if(value instanceof Map){
          if (((Map) value).containsKey("type")){
            element = parse((Map<String, Object>) value);
          }
        }else if (value instanceof List){
          element = new ArrayList<Object>();
          Object finalElement = element;
          ((List) value).forEach(item ->{
            ((ArrayList) finalElement).add(parse((Map<String, Object>) item));
          });
        }else {// 基本数据类型，直接设置。
          element = value;
        }
  ...
  public Field getField(List<Field> fields, String name){
    for (Field f: fields) {
